    ...
    public static class MongoDbStartup
    {
        ...
        private static void AddSampleUsersToMongo(UserManager<MongoUser> userManager)
        {
            ...
            foreach (var usrDummy in dummyUsers)
            {
                ...
                {
                    Claims = usrDummy.Claims.Select(c=> new IdentityUserClaim(c)).ToList(),
                ...
                if (!result.Result.Succeeded)
                {
                    ...
                    throw new Exception($"Error seeding the database! Make sure to drop all collections from Mongo before trying again!");
                }
            }
